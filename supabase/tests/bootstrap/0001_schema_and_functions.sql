...
-- ============================================================================
-- These three tables and their DDL were recovered from production.
-- Do not modify, infer constraints on, or add indexes beyond what is shown.
...
-- TEST-ONLY SYNTHETIC: booking_requests table
...
-- TEST-ONLY SYNTHETIC: notifications table
...
-- TEST-ONLY SYNTHETIC: booking_plans table
CREATE TABLE IF NOT EXISTS public.booking_plans (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  owner_id text NOT NULL
);

-- TEST-ONLY SYNTHETIC: users table (minimal for test)
...
-- Conversation members: only authenticated members can read
CREATE POLICY "conversation_members_select_shared" ON public.conversation_members
  FOR SELECT TO authenticated
  USING (public.is_conversation_member(conversation_id));

-- Messages: DM messages — members only
...
-- Messages: Event crew chat messages — crew members only
...
-- Message reads: users can read their own rows and co-participant rows
...
-- ============================================================================
-- PART 6: TABLE GRANTS/REVOKES (RLS-backed access control)
-- ============================================================================

-- Revoke broad access from anon role
...
-- Grant to authenticated; RLS policies restrict actual row access
...
-- DM creation only via RPC (direct inserts blocked)
...
-- ============================================================================
-- PART 7: RPC FUNCTIONS (with authorization checks)
-- ============================================================================

-- mark_conversation_unread: Internal helper; no PUBLIC/anon/authenticated EXECUTE
...
-- Explicitly revoke access from all client roles
...
-- cancel_event: Event owner only, marked with authorization checks
...
-- Explicitly revoke from anon; grant only to authenticated
...
-- delete_empty_event: Event owner only
