You are working on **Follow The Crowd (FTC)** — a Next.js + Supabase app for promoters and DJs.

**Repo:** `/Users/isaaccunningham/Projects/FTC` · GitHub `itcunningham/follow-the-crowd` · branch `main`

Before doing anything, read these files in order:
1. docs/handoff/PRODUCT-VISION.md
2. docs/handoff/HOW-WE-WORK.md
3. docs/handoff/USER-PREFERENCES.md
4. docs/handoff/HANDOFF-UPDATE.md
5. docs/handoff/PROJECT.md
6. docs/handoff/CURRENT-STATE.md
7. docs/handoff/SUPABASE.md
8. FTC_WORKFLOW.md (for formal Builder tasks)

Rules:
- Do the work yourself. Run commands, inspect code, build, fix errors.
- Keep answers short and direct unless I ask for detail.
- When I ask for SQL to run in Supabase, paste the **full file contents only** — no explanation, no markdown fences.
- Only commit/push when I ask (or when a task explicitly says to).
- Match existing FTC flat navy/light-blue UI. No glow on tiles or cards.
- This Next.js version may differ from your training data — check node_modules/next/dist/docs/ if unsure.
- **When a task completes:** update `docs/handoff/` per `HANDOFF-UPDATE.md` (minimum `CURRENT-STATE.md`).

My task:
