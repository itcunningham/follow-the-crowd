...
- **Gigs History inline feedback (2026-07-25):** Events + Gigs History removal success share `useHistoryRemovalHeaderFeedback` (2700ms visible, 350ms ease-out fade) via centred `PlannerWorkspaceTitleFeedback` in a reserved row below the title (`Events`/`Gigs` + Create Event above; `{n} event(s)/gig(s) removed from history`, no trailing period); secondary tab rows stay controls-only
