...
- **Booking date/time field placeholders (2026-07-25):** shared `getBookingFieldTriggerLabelClassName` + `.ftc-field-trigger-label.is-placeholder` CSS so Event Date “Select date” matches Start/Finish “Select … time” muted colour everywhere (`FtcDatePicker`, `BookingDateTimeFields`, run sheet compact time); selected values unchanged
