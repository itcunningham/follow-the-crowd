...
- **Chat reaction gestures (2026-07-29):** double-tap message bubble toggles ❤️ (touch + desktop double-click); long-press opens emoji tray with fade/scale animation (175ms ease-out) + dim backdrop; reactions apply optimistically; incoming layout uses fixed 3rem avatar column (`IncomingChatMessageLayout` two-row grid: avatar bottom-aligned on bubble row, reactions on row 2, nowrap timestamp under avatar); incoming reacted chips use neutral surface styling; position-aware grouping spacing; composer stays compact until wrap/newline; shared scroll padding tokens (`CHAT_MESSAGE_SCROLLER_CLASS`, `CHAT_MESSAGE_LIST_CLASS`)
...
- `e739fd8` — polish incoming chat row layout, reactions, composer, and scroll padding
