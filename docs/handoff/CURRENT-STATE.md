...
- **Event Plans selection cards (2026-07-19):** delete-selection renders only a top-right checkbox plus a layout-only action reserve (`EVENT_PLAN_ACTION_RESERVE_CLASS`); Use plan button is not in the DOM during selection mode, so title and plan details stay fixed without a visible duplicate button
