...
- **Chat reaction gestures (2026-07-29):** double-tap ❤️ toggles current user only; long-press picker; reaction badge in-flow footer on bubble frame lower-left for **all** directions (`CHAT_MESSAGE_REACTION_FOOTER_ALIGN_CLASS`, `CHAT_MESSAGE_BUBBLE_FRAME_CLASS`); real document height per message; `resolveSeenLabelSpacingClass` clears Seen when reactions present
