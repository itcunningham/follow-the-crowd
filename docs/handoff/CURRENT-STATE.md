...
- **Chat reaction gestures (2026-07-29):** reaction slot on bubble frame (`resolveMessageReactionSlotClass`: outgoing `left-0`, incoming `right-0`, shared `-mt-3` overlap); uniform within-group spacing — `first`/`middle` → `mb-1.5`, `last`/`standalone` → cluster end
