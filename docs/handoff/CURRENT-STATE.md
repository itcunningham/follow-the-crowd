...
- **Chat reaction gestures (2026-07-29):** double-tap ❤️ toggles current user only; long-press picker; reaction badge uses in-flow **footer** inside bubble frame (`CHAT_MESSAGE_REACTION_FOOTER_*`, `-mt-1.5` corner overlap) — real document height per message, no absolute overlay or zero-height hanger; compact pill unchanged
