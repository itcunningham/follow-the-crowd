...
- Event cards: compact title shows event name with venue on a dedicated secondary line in the mobile agenda; desktop month-grid and date-modal items use the same title → venue → time → status order; event fallback colour accent strip removed from agenda cards (colour remains on list/detail surfaces)
