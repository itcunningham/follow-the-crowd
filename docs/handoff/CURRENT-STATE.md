...
- **Withdrawal Other reason field (2026-07-25):** DJ withdraw / planner cancel “Other” details use `WithdrawalReasonDetailsField` — hard limits via `sanitizeWithdrawalOtherReasonInput` / `sanitizeWithdrawalOtherReason` (120 characters, 3 newline-separated lines max); Enter blocked at line 3; paste truncates to first 3 lines; bounded auto-grow (2–3 visible rows); live `n/120` counter; server path sanitizes in `cancelBookingRequest`; reason chips and submit logic unchanged
