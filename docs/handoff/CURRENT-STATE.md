...
- **Chat reaction gestures (2026-07-29):** double-tap ❤️ toggles current user only; long-press picker; reaction slot absolutely anchored lower-left on bubble frame (`CHAT_MESSAGE_REACTION_SLOT_CLASS`, `top-full -mt-2.5`) — zero list-height impact; uniform within-group spacing via `position` tokens (`middle`/`last` → `mb-1.5`, `first`/`standalone` → cluster end); `resolveSeenLabelSpacingClass` clears Seen when reactions present
