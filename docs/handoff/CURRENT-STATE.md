...
- **Calendar load perf (2026-07-22):** Events Calendar uses session/local item cache + workspace prefetch (`ensurePlannerCalendarItemsPrefetched`) with stale-while-revalidate on mount; no refetch on `date`/`month` URL-only changes; saved-plans hint reads Event Plans list cache when warm. Gigs Calendar uses matching cache/prefetch (`ensureDjGigsCalendarPrefetched`). Dual-role Events ↔ Gigs calendar tabs remount on switch (avoids `display: contents` hit-testing bugs); caches keep repeat opens fast. Workspace header uses `relative z-20 bg-ftc-bg` so primary tabs stay above calendar paint order on `/calendar`. Mobile agenda does not call `scrollIntoView` (month changes no longer shift document scroll); date strip centring uses horizontal `scrollLeft` only; dual calendar sub-tab switches preserve `window.scrollY`; active workspace and filter pills are no-ops on tap. Repeat Events ↔ Calendar workspace navigation shows cached grid immediately while background revalidate runs.
- **Gigs Calendar workspace tabs (2026-07-22):** Top workspace pills (Events / Event Plans / Gigs / Calendar) use the same `PlannerWorkspaceSubNavLink` + `buildWorkspaceSubNavDestinationHref` path as Events Calendar. On `/calendar`, stale `interceptWorkspaceTabNavigation` from Bookings Use Plan is ignored; sub-nav row stacks at `z-30`; dual calendar body at `z-0`; Gigs month-nav update pill overlay uses `min-h-9` (not full nav height). Leaving Calendar calls explicit `router.push` to canonical destinations (Gigs → Incoming, no calendar query carryover); active Calendar tab stays a no-op.
...
- **Events History controls (2026-07-20):** remove confirmation + selection toolbar live in fixed-height `EventsListTabRow` (feedback between pills and trash, ~3s fade); trash visibility uses `visibleFilteredEvents.length` (same list as cards), not cancelled-only subset
...
- `6937b8d` — fix workspace navigation from gigs calendar
