...
- **Calendar create workspace navigation (2026-07-25):** while calendar-originated create is open (`/calendar?create=…`), workspace sub-nav intercept stays active for the URL (not only while `createOpen`); leaving uses `window.location.assign` (App Router `router.replace` from create URL required two tab taps); Cancel still returns to the selected calendar date
