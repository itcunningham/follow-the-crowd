...
- **DM inbox image/attachment preview (2026-07-30):** image/file-only messages store empty `messages.text` (the file is a separate `message_attachments` row), so the inbox preview pipeline — which reads `message.text` as `latestPreview` — had nothing to render and fell through to "No messages yet" even though the conversation had real activity (order/timestamp/unread already worked, since those don't depend on text). Fixed by encoding a synthetic token (`lib/dmAttachments.ts`: `encodeDmAttachmentInboxPreview`/`parseDmAttachmentInboxPreview`, prefix `__ftc_dm_attachment__:`) into a message's `text` at the point messages are loaded/merged in `app/dm/page.tsx`, whenever real text is empty and a `message_attachments` row exists for it — same reuse pattern as the reaction-activity preview token. `formatDmInboxMessagePreview` renders it as "📷 Photo" / "📎 File"; a caption always wins over the token. **`message_attachments` is not in the `supabase_realtime` publication** (proven live), so live image-received updates are resolved via a narrowly-scoped direct `select` on `message_attachments` (one bounded retry after 700ms to cover the `messages`-insert-then-`attachment`-insert ordering gap in `sendDmMessageWithAttachment`) rather than a postgres_changes callback or polling.
