...
- **DM repeat-open scroll fix, attempt 3 (2026-07-31):** attempt 2 fixed reopen for Breaker Breaker (long, few/no images) but left FTC QA DJ (short, image-heavy) still landing wrong on reopen specifically — same `freshOpenToken` mechanism reliably re-triggers `scrollToBottom()` on every reopen for both, so the remaining gap had to be in what happens *after* that call, not whether it runs. Found a real asymmetry in `useChatScroll.ts`: the existing `loading`-transition reset effect sets `pinnedToBottomRef.current = true` synchronously (same tick), but the `freshOpenToken`/`isFreshOpen` path added in attempt 2 only set it as a side effect *inside* `scrollToBottom()`, deferred behind a `requestAnimationFrame`. The ResizeObserver effect (image-driven height growth re-pin) also depends on `messageIds`, so it tears down and re-subscribes in the same commit as a fresh open, and `ResizeObserver` delivers an observation immediately upon `.observe()`. If a component instance carries `pinnedToBottomRef.current === false` over from a prior scrolled-up read of that same conversation (isFreshOpen's whole reason for existing is exactly this kind of carried-over instance), that early observation — landing before the rAF — would see the stale `false` and skip re-pinning for any image-height growth it caught right then; image-heavy conversations are far more likely to have a height-changing layout event in that exact window than text-only ones, which fits why only FTC QA DJ (image-heavy) was still affected while Breaker Breaker (few/no images) was not. Fix: `pinnedToBottomRef.current = true` is now set synchronously in the `isFreshOpen` branch too, immediately after the suppression check, closing the async gap deterministically rather than by timing. One-line, additive; inert for the `needsInitialScrollRef`-only path (already true there) and for crew chat (`freshOpenToken` always undefined there, so this branch is reached the same way it always was). Not independently reproduced live (no authenticated session available in this environment) — this is a confirmed, provable code-level asymmetry with a fix that closes it deterministically, not a live-confirmed root cause; flagged for manual QA.
