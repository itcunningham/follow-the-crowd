...
- **Create Event DJ selection list height (2026-07-31):** the DJ selection `<ul>` in `app/(planner-workspace)/bookings/page.tsx` (`select-djs` step) used a flat `max-h-[420px]`; once a DJ's fixed-offer/ask-for-rate controls expanded inline, the remaining budget only showed a sliver of the next card before Send summary. First pass changed mobile to `max-h-[65dvh]`, but a plain percentage has no floor — on shorter real mobile viewports (address bar/safe-area chrome eating into `dvh`) it still collapsed to ~one expanded card + a sliver. Fixed with `max-h-[clamp(480px,72dvh,640px)] sm:max-h-[420px]`: measured real row heights (collapsed card 74px, selected+expanded card with rate controls 200px) and set the 480px floor to guarantee an expanded card plus 2 more regardless of on-device chrome, 72dvh as the scaling preference, 640px cap so very tall phones don't get an oversized panel; desktop (`sm:` and up) unchanged at flat 420px. List stays independently scrollable in normal document flow (no overlap with bottom nav or Send summary, which remain outside the scroll container)
...
- `25b7b12` — Create Event mobile DJ list: `65dvh` had no floor and still starved on shorter real viewports; switched to `clamp(480px,72dvh,640px)` (measured row heights, guarantees ~2-3 cards)
