...
- **DM reaction realtime (2026-07-29):** `message_reactions` added to `supabase_realtime` publication + `replica identity full` for DELETE payloads — migration `20250729100000_message_reactions_realtime.sql`; client channels `dm-reactions:{conversationId}` and `dm-inbox:reactions` now receive events; DM thread scopes events to loaded message IDs and dedupes via `upsertDmReactionFromRealtime`
