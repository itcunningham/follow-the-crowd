...
- **Withdrawal Other reason field (2026-07-25):** DJ withdraw / planner cancel “Other” details use `WithdrawalReasonDetailsField` — fixed ~3-row height with padded internal scroll (`scroll-padding`, reserved counter space); shared `sanitizeWithdrawalOtherReasonValue` (120 chars, first 3 `\n` lines); caret kept visible via `scrollWithdrawalReasonCaretIntoView`; live `n/120` counter
...
- `TBD` — polish withdrawal Details textarea scroll padding and caret visibility
