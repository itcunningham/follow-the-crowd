...
- **DM booking expand scroll (2026-07-28):** View details captures pre-expand scroll position before opening; expand align scroll runs once after transition; Hide details restores that pre-expand capture (not the elevated post-align scrollTop) so collapsed cards do not leave empty space beneath; `traceBookingCardCollapseScroll()` logs scroll metrics in dev
