...
- **Chat reaction gestures (2026-07-29):** double-tap ❤️ toggles current user only; long-press picker; reaction slot absolutely anchored lower-left on bubble frame (`CHAT_MESSAGE_REACTION_SLOT_CLASS`); messages above timestamps use cluster-end margin (`followedByTimeSeparator` → `CHAT_LIST_ITEM_CLUSTER_END_BEFORE_TIMESTAMP_SPACING_CLASS`) to clear reaction hang
