...
- **Events History delete-selection row (2026-07-23):** `EventsListTabRow` keeps fixed height; selection toolbar replaces the bin in the same right-anchored flex region with `HistorySelectionToolbar` `tabRowEmbedded` (outlined panel, `w-auto`, Back/All/Delete in a row expanding left from Delete) so cards do not shift when entering/exiting delete mode.
