...
- **Withdrawal Other reason field (2026-07-25):** DJ withdraw / planner cancel “Other” details use `WithdrawalReasonDetailsField` — fixed ~3-row height with internal scroll; shared `sanitizeWithdrawalOtherReasonValue` (120 chars, first 3 `\n` lines) runs on every textarea change; live `n/120` counter; no keyboard interception
...
- `e383d6e` — withdrawal Other textarea: value-only sanitiser, no keyboard interception
