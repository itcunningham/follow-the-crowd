...
- **Chat reaction gestures (2026-07-29):** double-tap ❤️ toggles current user only; long-press picker; reaction badge uses in-flow **hanger** below bubble (`CHAT_MESSAGE_REACTION_HANGER_*`, `-mt-2 h-0 overflow-visible`) with `pb-2` stack padding — hangs on outer corner, overlaps bubble edge only; no absolute overlay over text/Seen; compact pill unchanged
