...
- **Withdrawal Other reason field (2026-07-25):** DJ withdraw / planner cancel “Other” details use `WithdrawalReasonDetailsField` — fixed ~3-row height with internal scroll; hard 120-character limit; explicit 3-line cap via shared `sanitizeWithdrawalOtherReasonInput` (`beforeinput` rejects 4th Return, paste truncates); live `n/120` counter; wrapped text never blocked before 120 chars
...
- `TBD` — withdrawal Other textarea: reject 4th explicit line via beforeinput
