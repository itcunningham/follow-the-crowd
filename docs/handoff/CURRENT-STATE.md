...
- **Chat reaction gestures (2026-07-29):** double-tap message bubble toggles ❤️ (touch + desktop double-click); long-press opens emoji tray with fade/scale animation (175ms ease-out) + dim backdrop; reactions apply optimistically; incoming message groups use position-aware spacing (`first` / `middle` / `last` / `standalone`), middle `-mb-4` / last `-mb-3.5` tight stack in `flex-col-reverse`, avatar + timestamp footer tucked under final bubble (`-mt-1`, `leading-none`); attachments/booking cards break grouping via `groupable: false`; shared `buildChatMessageGroupLayout` + `resolveIncomingGroupLiClass` + `CHAT_INCOMING_MESSAGE_COLUMN_CLASS`
