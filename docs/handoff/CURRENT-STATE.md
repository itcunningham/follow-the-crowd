...
- **Gigs History inline feedback (2026-07-25):** Events + Gigs History removal success share `useInlineTabFeedbackDismiss` + `InlineTabFeedbackMessage` with Event Plans (`2700ms` visible, `duration-300` fade, clear at `3000ms`); centred `PlannerWorkspaceTitleFeedback` row below title; secondary tab rows stay controls-only
