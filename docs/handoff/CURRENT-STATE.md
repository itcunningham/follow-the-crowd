...
- **DM reaction realtime (2026-07-29): ⚠️ REQUIRES SQL — not live until Isaac runs it.** Proven by two-user test against production Supabase: `messages` realtime delivers to the recipient, `message_reactions` delivers **zero** INSERT/UPDATE/DELETE events, while the recipient *can* `SELECT` the same reaction row (so RLS is fine and the client channel is correct). Root cause: **`public.message_reactions` is not a member of the `supabase_realtime` publication** (`public.messages` is). Fix: run `supabase/migrations/20250729100000_message_reactions_realtime.sql` (adds the table to the publication + sets `replica identity full` so DELETE payloads and DELETE RLS checks work). Client side: `dm-reactions:{conversationId}` and `dm-inbox:reactions` were already correct; removed a local message-id gate that could silently drop a reaction arriving before its message was in local state; dedupe via `upsertDmReactionFromRealtime` / `removeDmReactionFromRealtime`
