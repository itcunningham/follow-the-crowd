...
- **DM booking expand scroll (2026-07-28):** unified `scheduleBookingCardExpandScrollTransition` coordinates expand/collapse; during the accordion transition scrollTop tracks scrollHeight shrink (no pre-expand capture or post-collapse restore jump); header align runs once after expand only; `traceBookingCardCollapseScroll()` logs scroll metrics in dev
