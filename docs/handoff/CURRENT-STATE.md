...
- **DM booking expand scroll (2026-07-28):** View details captures pre-expand scroll position before opening; expand/collapse accordion keeps the card visually stable via viewport-anchor compensation during the 200ms grid transition; align/restore runs only after transition end; Hide details restores pre-expand scroll position (not the elevated post-align scrollTop); `traceBookingCardCollapseScroll()` logs scroll metrics in dev
