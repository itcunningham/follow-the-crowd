...
- **Chat reaction gestures (2026-07-29):** double-tap ❤️ toggles current user only (add if absent, remove if present — never other users' reactions); long-press picker; `ChatMessageBubbleShell` with reaction-only gutter (`pb-3`), corner overlay (`top-full -mt-1.5 translate-y-0.5 left/right-0`), smaller neutral pill + enter/exit scale animation (0.8↔1.0, 160ms spring); outgoing Seen uses `mt-1.5` when reactions present so pill + Seen never collide
- **DM message grouping (2026-07-29):** consecutive same-sender text bubbles group via `buildChatMessageGroupLayout` (no time threshold — grouping is sender-only); incoming and outgoing share `resolveIncomingGroupTightMarginClass` / `resolveOutgoingGroupLiClass` (flex-col-reverse negative margins); avatar on cluster end only; stacked bubbles use `ftc-bubble-*-stack` connected-corner classes; cluster-end margin between sender groups; reactions do not affect grouping
...
- **DM composer multiline (2026-07-29):** message field is auto-growing `<textarea>`; Return/Enter inserts newline when current line has visible text (blank-line guard via `handleComposerNewlineKeyDown`); empty composer restores compact `h-11` single-line height; max ~5 lines then internal scroll via `useComposerTextareaAutogrow`; group chat composer matches
...
- `<pending>` — DM reaction positioning + Seen clearance gutter polish
