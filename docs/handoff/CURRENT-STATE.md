...
- **Events loading boundary (2026-07-22):** Route `loading.tsx` and auth `AppLoadingShell` for `/events` render **`EventsListAreaLoading`** (list skeleton only); workspace header + Active/History mount in **`EventsPageClient`**. Calendar-origin create still uses full **`EventsPageLoadingShell`**. Route-level chrome shell (`21ace05`) was **reverted** — it reintroduced the **`PLANNER_WORKSPACE_PAGE_SHELL_CLASS` TDZ** circular import via `EventsListTabControls` → `PlannerWorkspaceLayout`.
