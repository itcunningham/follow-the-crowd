...
- **DM reaction inbox activity (2026-07-30):** recipient inbox reactions use the existing `latestActivityAt` / `latestPreview` / `getUnreadConversationIds` pipeline. `dm-inbox:reactions` now handles DELETE: only if the deleted reaction is the current preview token, it restores the newest remaining valid reaction or the cached latest message, re-sorts, and recomputes unread from the replacement activity; deleting an older reaction leaves newer activity unchanged.
- **DM reaction realtime (2026-07-29):** `message_reactions` is in `supabase_realtime` with `replica identity full`; `dm-reactions:{conversationId}` and `dm-inbox:reactions` receive live events. The inbox DELETE path relies on the full old payload. Client dedupe uses `upsertDmReactionFromRealtime` / `removeDmReactionFromRealtime`.
