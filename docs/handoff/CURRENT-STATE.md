...
- **Gigs History inline feedback (2026-07-24):** removal success uses shared `useInlineTabFeedbackDismiss` + `EVENTS_LIST_TAB_FEEDBACK_CLASS` in the tab row (`1 gig removed from history` / `{n} gigs removed from history`, no trailing period) — no full-width card or list layout shift
