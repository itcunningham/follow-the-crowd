...
- **DM booking proposal card polish (2026-07-27):** when a pending proposal exists, expanded cards hide duplicate `Booking type` row; proposal card uses sentence-case **Proposed rate** label + amount; secondary action **Keep offer**; collapsed planner review uses label + amount (no duplicate `proposed` suffix); other roles see `$X proposed`; View event sits tighter below proposal review panel; shared `BookingProposalCardShell` / `BookingProposalCardAmount` exports in `BookingRateProposalPanel.tsx`
