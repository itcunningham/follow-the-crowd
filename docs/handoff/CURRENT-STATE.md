...
- **Gigs tab row (2026-07-19):** shared `DjGigsTabs` renders stable pill widths with reserved count slots (`GIGS_TAB_COUNT_SLOT_CLASS`) and History icon; loading shell uses the same component with `ready={false}` until gig counts resolve so tabs do not pop or shift on first paint
