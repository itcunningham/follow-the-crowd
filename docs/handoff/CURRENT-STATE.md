...
- **DM booking proposal card polish (2026-07-27):** when a pending proposal exists, expanded cards hide duplicate `Booking type` row; proposal card uses sentence-case **Proposed rate** label + amount; secondary action **Keep original offer**; collapsed summary shows `$X proposed` instead of generic ask-for-rate copy; View event sits tighter below proposal review panel; shared `BookingProposalCardShell` / `BookingProposalCardAmount` exports in `BookingRateProposalPanel.tsx`
