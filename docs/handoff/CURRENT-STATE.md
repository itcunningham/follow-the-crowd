...
- **DM booking expand scroll (2026-07-28):** `captureBookingCardExpandScrollContext` chooses **bottom-pinned** (within `CHAT_NEAR_BOTTOM_THRESHOLD_PX`) vs **anchor-preservation** before View details; bottom-pinned keeps captured bottom distance during accordion height changes and settles at `maxScrollTop` after Hide details; anchor-preservation keeps the card viewport Y during transitions and still allows one header align after expand when scrolled up; dev trace logs `scrollMode` on each phase
...
- `7869cf4` — preserve DM bottom position through booking accordion (bottom-pinned vs anchor-preservation modes)
