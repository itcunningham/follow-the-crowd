...
- **Chat reaction gestures (2026-07-29):** double-tap message bubble toggles ❤️ (touch + desktop double-click); long-press opens emoji tray with fade/scale animation (175ms ease-out) + dim backdrop; reactions apply optimistically via `applyOptimisticDmReactionToggle`; consecutive same-sender bubbles group with tighter spacing (`-mt-2`), avatar on cluster bottom only, timestamp cluster spacing (`mb-1.5`); shared `buildChatMessageGroupLayout`; picker portaled to `document.body` with measured flip/clamp; same gestures on event group chat — **SQL:** run `scripts/setupEventCrewChatReactions.sql` on Supabase for crew-chat RLS
