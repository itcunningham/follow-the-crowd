...
- **Calendar create workspace navigation (2026-07-25):** while calendar-originated create is open (`/calendar?create=…`), workspace sub-nav intercept stays active for the URL; leaving uses client `router.replace` (no full-page reload); Cancel still returns to the selected calendar date; create open no longer blocks on booking-plans fetch when opening Create event (plans load cache-first only for Event Plans picker)
