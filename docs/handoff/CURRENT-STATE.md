...
- **DM booking expand scroll (2026-07-28):** unified `scheduleBookingCardExpandScrollTransition`; collapse only adjusts scroll during the accordion animation (scrollHeight shrink compensation) with zero post-settle scroll writes; expand header align runs once after open; in-flight expand smooth scroll is aborted when collapse cancels the coordinator; dev trace logs each collapse `scrollTop` write via `scroll-write:*` phases
