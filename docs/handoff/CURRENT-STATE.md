...
- **Workspace sub-nav layout (2026-07-23):** Shared row `flex-nowrap`, stable `ftc-workspace-subnav-pill` border box, `key={tab.id}` on **`WORKSPACE_SUB_NAV_TABS`**. Gigs pill always mounts fixed-width badge slot (`w-[1.75rem]`); display count preserves cache/last-known through route transitions (`resolveWorkspaceGigsPendingDisplayCount`); no opacity placeholder digits.
...
- **Gigs tab row (2026-07-23):** `DjGigsTabs` — all three pills share `padding: 0.375rem 0.5rem` and stable active border box; Incoming/Confirmed counts cap at **99+** via `formatGigsTabCountDisplay`, with a fixed `2.75ch` right-aligned count slot; label/count gap **`gap-2` (~8px)**; workspace **Gigs** sub-nav badge uses the same formatter and reserved **99+** slot width.
