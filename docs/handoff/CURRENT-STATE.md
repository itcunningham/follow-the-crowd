...
- **Event Plans selection cards (2026-07-19):** delete-selection keeps the full-size Use plan button column via an invisible spacer (`EVENT_PLAN_USE_BUTTON_WRAP_CLASS` + `EVENT_PLAN_USE_BUTTON_CLASS`) so title and plan details never shift; compact checkbox overlays the card top-right corner (`top-3 right-3`) while the full card stays tappable
