...
- **DM chat scroll (2026-07-26):** auto-scroll runs only when a new message ID is appended (not on booking-card layout changes); booking expand scrolls the tapped card below the conversation header after its expand transition, with scroll locked during layout; collapse restores visual position; DM message list uses `overflow-anchor:none` (matches event chat) to prevent flex-col-reverse snap-to-latest on card resize
