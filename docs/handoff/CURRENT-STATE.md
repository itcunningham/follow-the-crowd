...
- **Gigs History inline feedback (2026-07-25):** removal success uses shared `useInlineTabFeedbackDismiss` (2700ms fade / 3000ms clear, same as Events History) in an absolute centred overlay in the planner title row (`Gigs` left, message centred; `1 gig removed from history` / `{n} gigs removed from history`, no trailing period, no truncate at 390px); secondary tab row stays Incoming/Confirmed/History/trash only
