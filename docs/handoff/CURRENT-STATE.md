...
- **Withdrawal Other reason field (2026-07-25):** DJ withdraw / planner cancel “Other” details use `WithdrawalReasonDetailsField` — hard limits via `sanitizeWithdrawalOtherReasonInput` / `sanitizeWithdrawalOtherReason` (120 characters, 3 explicit newlines server-side); client `measureWithdrawalReasonVisibleRows` + mirror textarea enforces 3 **rendered** rows (wrap-aware at 390px strict width); paste truncates deterministically; `overflow-y: hidden` (no scrollbar); live `n/120` counter; reason chips and submit logic unchanged
