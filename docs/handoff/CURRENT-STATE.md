...
- **DM booking card scroll anchoring (2026-07-26):** manual expand scrolls card top smoothly into view below the chat header; collapse preserves collapsed card position via visual anchor — user-initiated only (not realtime/load); bottom/last card gets dynamic message-list bottom padding so scroll can align when the container is at its scroll limit
