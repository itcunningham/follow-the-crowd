...
- **DM keyboard dismiss on scroll (2026-07-28):** on mobile viewports, deliberate downward drag in the message list dismisses the keyboard (upward drag never dismisses); scroll position preserved across blur; true interactive keyboard tracking unavailable on mobile web — see `dismissComposerKeyboardOnIntentionalScroll.ts`
