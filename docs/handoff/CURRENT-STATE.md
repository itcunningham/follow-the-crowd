...
- **Events History delete-selection row (2026-07-23):** `EventsListTabRow` keeps fixed height; selection toolbar replaces the bin in the same right-anchored flex region (expands left) with `HistorySelectionToolbar` `tabRowEmbedded` (no bordered panel padding) so cards do not shift when entering/exiting delete mode.
