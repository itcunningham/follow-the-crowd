...
- **Calendar load perf (2026-07-22):** Events Calendar uses session/local item cache + workspace prefetch (`ensurePlannerCalendarItemsPrefetched`) with stale-while-revalidate on mount; no refetch on `date`/`month` URL-only changes; saved-plans hint reads Event Plans list cache when warm. Gigs Calendar uses matching cache/prefetch (`ensureDjGigsCalendarPrefetched`). Dual-role Events ↔ Gigs calendar tabs keep mounted (`hidden`) after first visit so tab switches avoid remount refetches; tab wrappers use `display: contents` when active so flex `order-*` (alerts → mobile chrome → agenda) matches pre-perf layout. Repeat Events ↔ Calendar workspace navigation shows cached grid immediately while background revalidate runs.
