...
- **Create/edit validation:** inline field errors after Save / Continue; finish time shows "Finish time must be later than the start time." under Finish Time with red trigger border (`aria-invalid`); errors clear live when fixed; start + finish time both required; notes length/line limits disable save
