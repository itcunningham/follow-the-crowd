...
- **Chat reaction gestures (2026-07-29):** double-tap ❤️ toggles current user only; long-press picker; reaction badge anchored via `top-full -translate-y-[3px]` on bubble wrapper (hangs below corner, ~3px arc overlap); outer-edge inset `right-0.5` / `left-0.5`; zero document-flow height; compact pill unchanged
