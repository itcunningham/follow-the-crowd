...
- **Chat reaction gestures (2026-07-29):** double-tap ❤️ toggles current user only; long-press picker; reaction badge anchored to bubble wrapper outer bottom corner via `bottom-0` + `translate-y-[calc(100%-4px)]` (~4px arc overlap, remainder in inter-message gap); horizontal inset only (`right-2.5` / `left-2.5`); zero document-flow height; compact pill unchanged
- **DM message grouping (2026-07-29):** `buildChatMessageGroupLayout` + centralized spacing tokens (`CHAT_LIST_ITEM_*`, `CHAT_TIME_SEPARATOR_SPACING_CLASS`, `CHAT_SEEN_LABEL_*`); within-group gap `mb-1.5` via single `CHAT_LIST_ITEM_WITHIN_GROUP_SPACING_CLASS`; reactions absolutely anchored (`bottom-1`, `translate-y-[calc(50%-5px)]`) with zero document-flow height — badges overlap inter-message gaps without shifting stack rhythm; `resolveSeenLabelSpacingClass` keeps Seen clear when reactions present; composer placeholder guarded with `min-w-[5.75rem]`
