...
- **DM booking proposed-rate copy (2026-07-26):** planner review panel shows proposed amount only (no duplicate `Rate proposed` label); proposal notes reuse shared `BookingCardExpandableNotes`; primary action **Accept rate**; amounts formatted with `formatIntegerRateDisplay` (en-AU grouping)
- **DM booking proposal card polish (2026-07-27):** when a pending proposal exists, expanded cards hide duplicate `Booking type` row; proposal card uses sentence-case **Proposed rate** label + amount; ask-for-rate planner review uses **Decline rate** (fixed-rate keeps **Keep offer**); declining an ask-for-rate proposal clears the proposal, posts **Rate declined** system message, and returns DJ to propose-again state; collapsed planner review uses label + amount; shared `BookingProposalCardShell` / `BookingProposalCardAmount` exports in `BookingRateProposalPanel.tsx`
...
- **DM fixed chat document reset (2026-07-28):** returning to DM from Event Details resets document scroll/keyboard/nav-offset state before paint via `useFixedChatPageDocumentReset` so the composer does not jump above the bottom nav; Event Details Back uses `router.push(..., { scroll: false })`; DM composer placeholder `Message` (no ellipsis)
