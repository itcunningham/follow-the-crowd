...
- **Gigs tab counts (2026-07-19):** tab row shows a pill skeleton until the received-bookings snapshot resolves; labels and counts render together (no labels-first pop-in); background refresh keeps the last valid counts until the replacement snapshot is ready
