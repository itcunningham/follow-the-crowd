...
- **DM repeat-open scroll — FTC QA DJ, open (2026-07-31):** exhaustive hook-level testing (the harness above, plus every construction tried across three investigation rounds: reuse vs remount, image growth before/after the reopen's own rAF, short-vs-long lists) could not reproduce a failure in `useChatScroll` itself with the current code (attempt 1 + 2). If FTC QA DJ still fails to reopen on newest after this revert, the cause is outside this hook's scroll-timing logic — candidates not yet ruled out: something specific to that conversation's booking-card content (`dmBookingCardExpandScroll.ts` has its own height-animation system, distinct from plain `<img>` layout), or a `DmChatPage`-level state issue unrelated to `useChatScroll`. Needs one live manual check (see task report) before further investigation.
