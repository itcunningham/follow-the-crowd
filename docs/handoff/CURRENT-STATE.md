...
- **Chat reaction gestures (2026-07-29):** double-tap ❤️ toggles current user only; long-press picker; absolute reaction badge (`resolveMessageReactionAnchorClass` + shared inset/nudge/translate tokens) corner-arc overlap with zero list-item height impact; compact subtle pill (`px-0.5`, `h-3.5`, softened border/shadow) on `bg-ftc-bg-elevated/90`
