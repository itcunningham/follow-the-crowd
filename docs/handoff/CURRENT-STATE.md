...
- **DM message grouping (2026-07-29):** `buildChatMessageGroupLayout` + centralized spacing tokens (`CHAT_LIST_ITEM_*`, `CHAT_TIME_SEPARATOR_SPACING_CLASS`, `CHAT_SEEN_LABEL_*`); within-group gap `mb-1.5` via single `CHAT_LIST_ITEM_WITHIN_GROUP_SPACING_CLASS`; list uses `gap-0` with per-`li` margins via `resolveMessageGroupLiClass`; reaction overlap reserves `CHAT_MESSAGE_REACTION_OVERLAP_RESERVE_CLASS` + `resolveSeenLabelSpacingClass` keeps Seen/delivered clear of corner badges; composer placeholder guarded with `min-w-[5.75rem]` (no `overflow-hidden` truncation)
