...
- **Events loading boundary (2026-07-22):** `/events` **`layout.tsx`** mounts persistent chrome via **`EventsWorkspaceRouteShell`** (title, Create Event from guard/cached role, workspace tabs, shared **`EventsListTabControls`**). Route **`loading.tsx`** and auth **`AppLoadingShell`** render **`EventsListAreaLoading`** (list skeleton only). **`EventsPageClient`** bridges list state through **`EventsWorkspacePageBridge`**; calendar-origin create still uses full **`EventsPageLoadingShell`**.
...
- `21ace05` — stabilise complete events loading chrome
